...
void pivoteEsc(float a[][100],int ecu,int inc);
...
void main()
{
	...
	do
    {
        ...
        printf("5) Metodo Pivote Maximo\n");
        printf("6) Metodo Pivote Escalonado\n");
        printf("7) Salir\n");
        printf("Opcion: ");
        scanf("%d",&menu);
        switch(menu)
        {
            ...
            case 5: system("cls");
                    ...
                    break;
            case 6: system("cls");
                    pivoteEsc(ma,ecuaciones,incognitas);
                    break;
            case 7:
                    printf("\n\nSaliendo...");
                    break;
...
void pivoteEsc(float ma[][100],int ecu,int inc)
{
    float aux[100][100], s[100], m[100][100], temp;
    float x[100],sum;
    int i,j,n,p,max,k,nfila[100];
    n = ecu;
    for(i=1; i<=n; i++)
        for(j=1; j<=n+1;j++)
            aux[i][j]=ma[i][j];
    ...
    for(i=1;i<=n;i++){
        for(j=1;j<=n;j++)if(fabs(aux[i][j])>s[i])s[i]=fabs(aux[i][j]);
        if(s[i]==0){
    ...
    for(i=1;i<n;i++){ //paso 2
            p=i;
        for(j=1;j<=n;j++){
            if((fabs(aux[nfila[p]][i])/s[nfila[p]])<(fabs(aux[nfila[j]][i])/s[nfila[j]]))p=j;
        }
        if(aux[nfila[p]][i]==0){
            printf("no existe solucion unica");
            return;
        ...
        for(j=i+1;j<=n;j++){
            m[nfila[j]][i] = aux[nfila[j]][i]/aux[nfila[i]][i];
            for(k=1;k<=n+1;k++){
                aux[nfila[j]][k] = aux[nfila[j]][k] - m[nfila[j]][i]*aux[nfila[i]][k];
            }
        }
        imprime_m(aux,ecu,inc);
    }
        ...
        x[n]=aux[nfila[n]][n+1]/aux[nfila[n]][n];

        for(i=n-1;i>=1;i--){
            sum=0;
            for(j=i+1;j<=n;j++)sum+=aux[nfila[i]][j] * x[j];
            x[i]=(aux[nfila[i]][n+1]- sum)/aux[nfila[i]][i];
        }
